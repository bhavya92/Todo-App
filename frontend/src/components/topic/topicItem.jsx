...
export default function TopicItem() {
    ...
    {topic === null ? (
              ...
              topic.map((item) => (
                    <div className=" flex flex-row bg-white-200 rounded-md 
                                w-full h-fit px-3 py-2 mt-3 cursor-pointer
                                transition-all duration-100 ease-in-out
                                hover:shadow-lg hover:shadow-white-600
                                hover:bg-white-300"  key={item._id} onClick = { ()=> setTopicToFetchHandler(item._id) }>
            ...
            <div className="basis-4/6 flex flex-row justify-between">
                ...
                <span className='p-1 hover:scale-110' onClick={()=>deleteTopicHandler(item._id)}>
